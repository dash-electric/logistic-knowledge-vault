...
- Invariant enforcement: pickup-before-drop-off per shipment, one active route per shipment, drop-off blocked until pickup completes.
- Internal API: create route, list, detail, add stops, resolve stop (collection: `Logistic Service/Internal/Route/`).
...
- `DIRECT_4W` vehicle checks: a fixed 8-item evidence form (selfie, vehicle front/back/left/right, inside box, odometer photo, odometer reading) required at route start and again at route end, gating stop execution and route completion.
...
- Rider-app execution surface (native screens or trip-service bridge) — deferred to a follow-up version once the execution decision (Open questions) lands.
...
9. Route status derives from its stops: `PLANNED` → `IN_PROGRESS` (first stop transition) → `COMPLETED` (all stops terminal) | `CANCELLED` (explicit, only actionable while un-departed stops remain).
...
- **Permission boundary:** all v1 endpoints are internal-token only; no portal or rider exposure yet.
- **All stops done but no end check (`DIRECT_4W`):** the route stays `IN_PROGRESS` until the end check is submitted — completion is derived, never forced. Ops sees the gap as a stuck-in-progress route (alert candidate below).
- **Route cancelled mid-journey (`DIRECT_4W`):** no end check required; a submitted start check is retained.
- **Odometer lower at end than start:** rejected `422 ODOMETER_REGRESSION` — a lower reading means a typo or a swapped vehicle; both need a human, not silent acceptance.
...
- Zero invariant violations in production data: no orphan drop-offs, no shipment on two active routes, no drop-off completed before its pickup.
- Post-ship review can trace any shipment's route history from `ROUTE_STOP_SHIPMENTS` alone.
...
Add a self-contained route module to `nest-logistic-service`: four new tables (`routes`, `route_stops`, `route_stop_shipments`, `route_stop_deliveries`), an internal REST API for creating and resolving routes, and invariant enforcement in the write path. The model mirrors trip-service's proven trip/stop/deliveries shape but at shipment grain, with the vault's snapshot pattern for the rider master and batch-style route-geometry snapshots. Nothing existing changes: batches, the delivery bridge, and dispatch keep working as-is.

## Architecture

- **Owner:** `nest-logistic-service`, new `route` module (schema + usecases + internal controller), same layering as dispatch/invoice modules.
...
Collection: `Logistic Service/Internal/Route/` (added in this change). Auth: internal service token, inherited from collection. Base: `{{baseUrlLogistic}}`.

```
POST  /internal/v1/routes
GET   /internal/v1/routes
GET   /internal/v1/routes/:routeID
POST  /internal/v1/routes/:routeID/stops
PATCH /internal/v1/routes/:routeID/stops/:stopID/resolve
PATCH /internal/v1/routes/:routeID/vehicle-check/:phase    (phase: start | end — DIRECT_4W only)
```

**Create route** — `POST /internal/v1/routes`
...
**List routes** — `GET /internal/v1/routes?riderID=&status=&shipmentType=&date=&page=&pageSize=` → paged summaries (no stop bodies). (`hubID` filter arrives with hub attribution in v2.)

**Route detail** — `GET /internal/v1/routes/:routeID` → full route: stops in sequence, shipments per stop, transition history per stop, `arrivedAt`/`resolvedAt` execution timestamps, and per-stop leg geometry (`legPolyline`, `legDistanceM`, `legDurationSec` — null until the route geometry is computed) alongside the route-level `routeOptimization` snapshot.

**Add stops** — `POST /internal/v1/routes/:routeID/stops` → same stop shape as create; only while `PLANNED`/`IN_PROGRESS`; appended after the last resolved stop; whole-route invariants re-validated. Returns the full updated stop list.

**Resolve stop** — `PATCH /internal/v1/routes/:routeID/stops/:stopID/resolve`
...
{ "status": "COMPLETED",              // COMPLETED | FAILED | SKIPPED
  ...
  "resolvedBy": "svc-rider-bridge" }
```

`arrivedAt` may also be reported ahead of resolution once an execution surface exists (deferred with that surface); in v1 the resolve call carries it.

Errors: `409` out-of-order resolution (earlier unresolved stop exists), `422` drop-off before its pickups completed, `422 VEHICLE_CHECK_REQUIRED` on a `DIRECT_4W` route whose start check is missing (Req 15). Side effects: shipment status transitions, dependent drop-off blocking (Req 7–8), route status derivation (Req 9 + Req 16).

**Submit vehicle check** — `PATCH /internal/v1/routes/:routeID/vehicle-check/:phase` (`start` | `end`; `DIRECT_4W` routes only)
...
All 8 items required, image values are single URLs (trusted media host allowlist, same rule as stop workflow evidence). Response `200`: the stored check (+ `distanceKm` on the `end` phase: end minus start reading). Errors: `409` not a `DIRECT_4W` route / check already submitted / `end` before `start` / route not in a submittable state; `422` missing item or `ODOMETER_REGRESSION` (end < start).
...
- **`routes`** — id, unique `code`, unique nullable `idempotency_key`, `shipment_type` (`DOCKING`|`DIRECT_2W`|`DIRECT_4W`, default `DOCKING`), `rider_id` (text) + `rider` JSONB snapshot `{id, code, name, phone_number}`, nullable `external_shift_id`, nullable `external_trip_id` (hub attribution deferred to v2 — no hub columns in v1), server-maintained totals, `status` (`PLANNED`|`IN_PROGRESS`|`COMPLETED`|`CANCELLED`, default `PLANNED`), `start_check`/`end_check` (JSONB, nullable — the `DIRECT_4W` 8-item vehicle-check evidence `{selfie, vehicleFront, vehicleBack, vehicleLeft, vehicleRight, vehicleBox, odometerPhoto, odometerReading, submittedAt, submittedBy}`; always null for other types), route-geometry snapshot (polyline/distance/duration/computed_at), `started_at`/`completed_at`/`cancelled_at`/`cancellation_reason`, timestamps.
...
- **Trip-service:** no calls in v1; `external_trip_id` reserves the bridge.
...
- New vehicle-check signals: check submissions by outcome, time from route creation to start check, stuck-in-progress `DIRECT_4W` routes (all stops terminal, no end check — alert candidate), odometer-regression rejections (fleet data-quality signal).

## Security & permissions

- v1 endpoints are internal-token only (collection-level bearer, `secretKey` env var — never committed).
- Stop payloads carry PII (addresses, contact phones) — same handling rules as shipments; internal responses may include them, but any future portal/rider exposure gets its own review.
- Input validation at the DTO boundary: enum-checked types/statuses, lat/long ranges, `sequence` integrity, shipment UUIDs must exist and belong to the caller's scope.

## Rollout

1. **Increment 1 (safely deployable alone):** migration + tables + `POST/GET` endpoints behind an env feature flag (`ENABLE_ROUTE_MODULE`); no producers, no consumers — zero behavior change.
2. **Increment 2:** add-stops + resolve + shipment-status side effects (needs the status-mapping open question resolved).
3. **Increment 3 (separate PRD/TRD version):** execution surface (rider app or trip-service bridge) and portal UI + mockup.

No backfill. Rollback = disable flag; tables are inert without traffic.

## Testing strategy

- **Unit:** invariant validators (pickup-before-drop-off incl. add-stops and reorder, one-active-route, pickup-and-drop-off-required-per-shipment, blocked-drop-off derivation), route status derivation, totals computation, dwell/leg-time timestamp ordering (`arrived_at` ≤ `resolved_at`).
- **Integration:** create→resolve happy path P,D,P,D and P,P,D,D orderings; failed-pickup → blocked/skipped drop-off cascade; mid-route shipment cancellation; idempotent replay; concurrent create racing the same shipment (unique-index backstop).
- **Contract:** responses match the collection examples in `Logistic Service/Internal/Route/` (success + failure per endpoint).
...
- **Execution surface:** native rider-app support for routes vs. bridging each route to a trip-service trip. (Trip-service already has the driver endpoints; bridging avoids a second resolution engine but adds a sync problem.)
...
- **`DIRECT_2W` checks:** 2W routes currently require nothing at start/end — should they get a lighter check (e.g. selfie + odometer) later?
- **Vehicle identity:** the check photographs a vehicle but records no vehicle id/plate — worth capturing once fleet master data is wired in?
