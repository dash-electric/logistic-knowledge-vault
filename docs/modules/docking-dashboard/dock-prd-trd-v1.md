...
- 2026-07-12 — backend implemented on `feat/dashboard-v2` (worktree branch off origin/main): pre-flight A done (merge + migration regenerated as 0047 + nullable-origin drift fix), pre-flight B verified (existing `AuthGuard` + ADMIN auth with `providerPitstopIds` scoping), asOf param, volumeStress, stage-timeline endpoint, coverage excludedDays, unknown-hub 404s, per-hub permission guard (closed an IDOR gap: per-hub endpoints ignored `permittedHubIds`), PARTIAL-day exclusion; 25 unit tests. Deferred at implementer's discretion: the post-ETL stats cache (compute-on-read measured trivial at current scale — add when p99 threatens). Still pending: pre-flight C (external scheduler), backfill run, golden reconciliation in staging.
