...
1. A client has 0+ workflow definitions, at most **one ACTIVE** at a time; a workflow has 1+ ordered milestones; a milestone has 1+ ordered fields.
...
3. At route creation (and add-stops), each stop resolves a workflow: if all shipments on the stop belong to one client and that client has an ACTIVE workflow, its full definition plus empty submission state is snapshotted into `route_stops.workflow`; otherwise `workflow` is `null`.
...
7. A submission is validated against the snapshot: all required fields present, types correct, image URLs count matches `imageCount` exactly (or ≥ 1 when `multiple`), bounds respected. Validation failures reject the whole submission.
8. A completed milestone records `submittedAt` and `submittedBy` and cannot be resubmitted (`409`) — evidence is append-only in v1. Every accepted submission is durably persisted as a `WORKFLOW_SUBMISSIONS` row (stop, workflow, milestone key, values, who, when) in the same transaction as the snapshot update — the submissions table is the system of record; the snapshot is the read model.
...
- **Duplicate field/milestone keys in a definition:** rejected at definition create/update time (`422`), not at run time.
...
Collection: `Logistic Service/Internal/Workflow/` (added in this change). Auth: internal token. Base: `{{baseUrlLogistic}}`.

```
POST  /internal/v1/stop-workflows
GET   /internal/v1/stop-workflows
GET   /internal/v1/stop-workflows/:workflowID
PUT   /internal/v1/stop-workflows/:workflowID
PATCH /internal/v1/routes/:routeID/stops/:stopID/milestones/:milestoneKey
```

**Create workflow** — `POST /internal/v1/stop-workflows` (full nested definition, atomic):
...
```jsonc
{
  ...
  "milestones": [
    {
      "key": "arrival-evidence",
      "name": "Arrival evidence",
      "sequence": 1,
      "fields": [
        { "key": "outlet-front", "label": "Outlet front photo", "type": "IMAGE", "required": true, "config": { "imageCount": 3 } },
        { "key": "receiver-name", "label": "Receiver name", "type": "TEXT", "required": true, "config": { "maxLength": 120 } }
      ]
    },
    {
      "key": "handover",
      ...
      "sequence": 2,
      "fields": [
        { "key": "goods-photos", "label": "Goods photos", "type": "IMAGE", "required": true, "config": { "multiple": true } },
        { "key": "received-qty", "label": "Quantity received", "type": "NUMBER", "required": true, "config": { "min": 0 } }
      ]
    }
  ]
}
...
Errors: `409` client already has an ACTIVE workflow (deactivate first or send `status: "INACTIVE"`); `422` duplicate keys / empty milestones / invalid config (e.g. `imageCount` and `multiple` together).

**List** — `GET /internal/v1/stop-workflows?clientID=&status=` → summaries. **Detail** — `GET .../:workflowID` → full nested definition. **Update** — `PUT .../:workflowID` → full replace of the nested definition (same validation as create); in-flight snapshots unaffected.
...
{
  "values": {
    "outlet-front": ["https://media.dash.co/a.jpg", "https://media.dash.co/b.jpg", "https://media.dash.co/c.jpg"],
    "receiver-name": "Ibu Sari"
  },
...
- **`stop_workflows`** — id, `client_id` (integer, external core-service client, no FK), `name`, `status` (`ACTIVE`|`INACTIVE`, default `ACTIVE`; partial unique index: one ACTIVE per client), timestamps.
- **`workflow_milestones`** — id, `workflow_id` FK CASCADE, `sequence` (unique per workflow), `key` (unique per workflow), `name`, timestamps.
- **`workflow_fields`** — id, `milestone_id` FK CASCADE, `sequence` (unique per milestone), `key` (unique per milestone), `label`, `type` (`IMAGE`|`TEXT`|`NUMBER`), `required` (bool, default true), `config` JSONB nullable (`{imageCount}` xor `{multiple}` for images; `{maxLength}`/`{min}`/`{max}` for text/number), timestamps.
...
```jsonc
{
  ...
  "milestones": [
    { "key": "arrival-evidence", "name": "Arrival evidence", "sequence": 1,
      "status": "COMPLETED",                    // PENDING | COMPLETED
      "submittedAt": "2026-08-04T03:04:10Z", "submittedBy": "svc-rider-bridge",
      "fields": [
        { "key": "outlet-front", "label": "Outlet front photo", "type": "IMAGE",
          "required": true, "config": { "imageCount": 3 },
          "value": ["https://…/a.jpg", "https://…/b.jpg", "https://…/c.jpg"] },
        { "key": "receiver-name", "label": "Receiver name", "type": "TEXT",
...
- **m-app (rider surface):** consumes the snapshot shape and the null-fallback contract; its implementation is out of scope here.
...
- Definition inputs validated at the DTO boundary: enum types, key format (kebab-case, unique), config schema per type, sequence integrity.

## Rollout

1. **Increment 1:** definitions — migration (3 tables) + CRUD endpoints behind `ENABLE_STOP_WORKFLOWS`. Inert without consumers.
2. **Increment 2:** execution — `route_stops.workflow` column + resolution at route create + submit endpoint + resolve gate. The route module's flag gates route creation itself, so this composes with `ENABLE_ROUTE_MODULE`.
3. **Increment 3 (separate version):** portal authoring UI + mockup; m-app rendering ships on the app's own train.

Rollback: disable flag; the nullable column and tables are inert. No backfill anywhere.
