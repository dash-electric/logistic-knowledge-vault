...
erDiagram
  ...
  ujp {
    ...
    boolean charged_positioning_override
...
Nested DTO groups mirror the wizard steps: `header` (clientId, deliveryDate, isReverse, serviceType, deliveryType, shift) · `payee` (type, subconVendorId?, bankName, accountNumber, accountHolder, nominalTransfer) · `vehicle` (plateNumber, unitType, energyType, fuelType, baseline or konsumsiPerKm, energyPrice, eMoney) · `costs` (kmYangDiajukan, bbmFixOverride, tollFlazz, parkirTapMachine, parkirManual, biayaBongkarMuat, biayaLainLain, justifikasiBiayaLainLain, uangMakan) · `route` (routePlanId, stops, legs, **tariffConfigId?, ringId?** — the tariff tag rides with the route group, CR-4) · `rider` byte-identical to the 4W DTO.
...
```
{
  header:       { id, referenceId, status, version, clientId, client, deliveryDate, isReverse,
                  serviceType, deliveryType, shift,
  ...
  totals:       { totalKmWithMargin, kmMarginPct, estimasiBbmLiter, totalBbmCost,
                  ...
                  amended*, totalWithAmendments* },                 // CR-7 · Σ APPROVED biaya tambahan; * cost tier
  amendments:   [{ id, seq, amount*, justifikasi, status: SUBMITTED|APPROVED|REJECTED,
                   requestedBy, requestedAt, decidedBy, decidedAt, decisionNote, reasonCode }],   // CR-7
  revenue:      { amount, margin, status, note, tariff } | null,   // CR-4 · APPROVER ONLY (absent/null otherwise)
  estimate:     { totalKmWithMargin, estimasiBbmLiter, totalBbmCost*, kmAllLegs, kmCharged, kmMarginPct,
                  chargedPositioning, reverseChargeApplied* },   // * cost tier: null for a non-party
  rejection:    { reasonCode, reasonLabel, note, decidedBy, decidedAt } | null,
  masked:       boolean,
  history:      [{ at, by, from, to, note, reasonCode, changes: [{ field, from, to }] | null }],
  shipment:     { id, waybill, routeCode, status } | null,
  shipmentSkipped: 'SUBCON' | null,
  viewer:       { isRequester, isApprover, canApprove, canCancel, canEdit, isPriceViewer,
                  canAmend, canDecideAmendment },  // isPriceViewer = isApprover (CR-4); CR-7 flags
...
| **approve: margin below zero on a known revenue** | loss trip | unit | `409 NEGATIVE_MARGIN_CONFIRM_REQUIRED` unless `confirmNegativeMargin`; sign stored | "Margin negatif" confirm |
...
| Unit (web, **CR-4**) | `ujp/tariffs/index.test.tsx` (approver sees the list + edit; a non-approver sees a locked notice, no rates; create per tipe; rings edited under a PER_RING config) · wizard step Rute **ring picker** (PER_RING client → auto-suggested ring pre-selected + editable; plan default pre-fills; non-PER_RING → no picker) · panel **margin section** by persona (approver sees revenue/margin + the `revenueStatus` banner; requester and other see the cost with the price locked — asserts the price is absent from the DOM, not merely hidden) · `revenueStatus` banners (belum bisa dihitung / MULTI_RATE_FALLBACK) · decision **`409 NEGATIVE_MARGIN_CONFIRM_REQUIRED` → "Margin negatif" confirm → resend with `confirmNegativeMargin`** · list missing-ring badge + approver-only margin cell · axe on the tariff page and the margin section |
| Contract (**CR2-D11**) | `services/api/schemas/{ujp,routePlans,addresses,tariffs}.ts` infer the TS types and guard `unwrap()` in dev/test; `collection-contract.test.ts` validates **every** response example under `dash-api-collections/.../Logistic/{UJP,Route Plans,Addresses,Tariffs}` (path via env, skipped when absent) and runs in CI. `Detail UJP` first — the flat-versus-grouped mismatch found 2026-09-17 is the motivating bug. **CR-3 extends the same schemas**: `header.version`, `viewer.canEdit`, the optional `history[].changes`, the `PUT` response, and the `expectedVersion` field on the decision body — plus the new `Update UJP.yml` examples. **CR-4 adds** the `tariffs.ts` schema + the six `Logistic/Tariffs/` examples, the `tariffConfigId`/`ringId` create/update fields, the approver-only `revenue` block on `Detail UJP` (with a non-approver example asserting it is absent), and the `revenue*` + `409 NEGATIVE_MARGIN_CONFIRM_REQUIRED` examples on the approve request |
...
| CR4-D7 | **Negative margin = warn + explicit approver confirm + store the sign** (not a hard block, unlike source). Server-enforced: an `OK`-revenue approve with `margin < 0` is refused `409 NEGATIVE_MARGIN_CONFIRM_REQUIRED` without `confirmNegativeMargin`. The confirm fires **only** when revenue is actually known (CR4-D10) — a low/negative margin on unreliable revenue is never presented as a decision. |
...
Consequences elsewhere in this document: §1 (summary rows), §2.2/§2.3 (the `tariff` module + arrow), §2.4 (ERD), §2.6 + §2.10 (revenue resolved and snapshotted inside approve), §3.1 (`tariffConfigId`/`ringId` on create/PUT, the approve response `revenue*` + `NEGATIVE_MARGIN_CONFIRM_REQUIRED`), §3.4 (approver-only `revenue` block + `isPriceViewer`), §3.5 (the tariff endpoints), §4.3 (migration `0097`) + §4.4 (tables), §5 (margin uses the one cost), §7 (field-tier + tariff gating + server-side confirm), §9/§10 (web + cross-module), §11/§12 (failure modes + specs), §13 (lanes P1–P5).
...
- 2026-09-21 — **v3.6**: CR-8 pool-leg billing per stop (assessment §24, CR8-D1…D6). Pool stops carry `charged: boolean | null` (route-plan and UJP stop DTOs; null = client rule; the more specific wins, both directions). `kmCharged` = Σ legs whose two endpoints are billed (`RouteStopKindHelper.isCharged`), `kmProposedDefault = kmCharged`; the UJP stores `charged_positioning_override` (folded into `0097`) and the detail/estimate expose `chargedPositioningApplied/Override`. **`shipmentStops` returns every stop** — billing never drops a pool stop from the DIRECT_4W route, so its `arrived_at/resolved_at` are recorded; an unpinned pool stop resolves to the slot default or no workflow (never blocks). Web 4W "Isi dari rute" keeps every stop too. Supersedes CR-D7 / CR2-D7's "inner stops only for an uncharged client". **CR-8b (same day):** the web no longer has a "Leg pool" switch — the pool is the first and last stop by position, the only control is a "Ditagih ke client" toggle on those two stops (prefilled from the client rule), and `pool` is derived as `isEdge && !effectiveCharged` before it is sent; the DTO/contract is unchanged. The server applies the same position rule (`RouteStopKindHelper.deriveBilling`, CR8-D8) in `UjpCostService` and `RoutePlanService.kmCharged`, ignoring the FE `pool` flag for billing; `backfill-route-plan-edge-charged.sql` pins pre-CR-8b real edge stops as `charged = true`.
- 2026-09-21 — **v3.5**: CR-7 biaya tambahan (assessment §23, CR7-D1…D5). New table **`ujp_cost_amendments`** (folded into `0097_ujp_cr1_to_cr6`): a child of an APPROVED UJP for a cost the trip incurred after approval; the approved request, its first payout and `margin_amount` snapshot are never edited. `POST /v1/ujp/:id/amendments` (any ops; APPROVED, shipment not COMPLETED, one SUBMITTED at a time, amount > 0, justification mandatory) and `POST …/amendments/:amendmentId/decision` (approver allowlist, never the raiser, reject = ACTIVE `UJP_REJECTION` reason). `GET /v1/ujp/:id` gains `amendments[]`, `totals.amended` / `totalWithAmendments` (cost tier), `viewer.canAmend` / `canDecideAmendment`; `GET /v1/ujp` rows gain `pendingAmendments`. Every write appends a `ujp_status_history` row APPROVED → APPROVED with note `BIAYA_TAMBAHAN_*` and a `changes` diff. No driver push. Also fixed: `getShipmentSummary` had an ambiguous `id` in the route-code subquery (500 on any approved detail).
- 2026-09-21 — **v3.4.1**: dead inputs dropped and a rekening prefill added. **Fix:** `GET /v1/ujp/:id` `estimate` now carries `totalBbmCost*`, `kmAllLegs`, `kmCharged`, `kmMarginPct`, `chargedPositioning`, `reverseChargeApplied*` as the collection already documented (the panel read "— / — / n km" and "margin 0%" from the two-field block; §3.4 corrected). Same sweep, found by the new BE-side collection contract spec (`ujp-collection-contract.spec.ts`, example ⊆ response): `header.clientId/clientName`, `payee.subconVendorId`, `clientConfig.defaultEMoney` were missing (edit mode lost client/vendor), stops now always carry `type/intent/pool/role` + explicit null pointers, legs a `source`, and `history` is returned as `at/by/from/to` (the panel showed "undefined oleh —"). `ujp.ops_team`, `ujp.jam_mulai`, `ujp.jam_selesai` and the six cargo columns (`sender_name/phone`, `receiver_name/phone`, `item_name`, `bobot`) dropped — the `cargo` DTO group and `GET` block are gone (nothing read them: the 4W shipment takes sender/receiver from the stops). The wizard's bank block is prefilled from the **driver master**: web calls nest-driver-service `GET /v3/drivers/:id` (already returns `bank.name`, `bank_account_name`, `bank_account_number` from `driver_bank`, WEB/ADMIN tokens allowed) and `applyRiderBank` fills only an untouched block — no logistic endpoint needed. Also (`header.jamMulai/jamSelesai` removed from DTOs and responses; they were logisticdash schedule fields with no reader here) (the requester is `requester_email` from auth; no ops-team master). `header.opsTeam` removed from `POST/PUT /v1/ujp` and from `GET /v1/ujp[/:id]` responses; the edit change-list no longer tracks it. Folded into the single feature migration **`0097_ujp_cr1_to_cr6`** (regenerated) rather than a follow-up migration, since the branch is unmerged.
- 2026-09-21 — **v3.4**: CR-6 (assessment §22, CR6-D1…D6). Stop = `type` (WorkflowStopType) + `intent` (WorkflowStopIntent, widened to GENERIC) + `pool`; `role` DERIVED and kept on every persisted stop (`RouteStopKindHelper.kindOf/roleOf/normalize`, `@infrastructure/helpers`) — no schema migration; idempotent backfill `src/database/backfill-route-plan-stop-kind.sql`. Rules re-expressed on `pool`: `kmCharged` (route + UJP cost), `shipmentStops`, validity (≥1 non-pool PICKUP and DROP_OFF). Approve maps `type`/`intent` 1:1 (pool DROP_OFF · GENERIC → shipment DROP_OFF without intent; RETURN_CONTAINER only when set). Pin service uses `kindOf(stop).type`. DTOs (`RoutePlanStopDto`, UJP route stop) accept `type/intent/pool` or legacy `role` (one required, 400 otherwise); responses carry all four. CI: `stg-deployment.yml` in both repos passes `UJP_APPROVER_EMAILS`/`UJP_KM_MARGIN_PCT` and `REACT_APP_UJP_APPROVER_EMAILS` from the `staging` GitHub environment (values set 2026-09-21). ERD annotated in the same change.
- 2026-09-20 — **v3.3**: CR-5 built (assessment §21.1): `route_plans.stops[].workflow_id/workflow_name` (jsonb, no migration), `RoutePlanWorkflowPinService` (exists / client / ACTIVE / role→stopType, name denormalised on save; 400 `WORKFLOW_*`), `Direct4WCreationService.resolveWorkflow` split into `pinProblem` + `resolveDefault` + `freeze` — manual path 400 as before, plan-sourced (`allowWorkflowFallback`) → slot default + snapshot flags `fallback/pinnedWorkflowId/fallbackReason` (inside `route_stops.workflow`, no column); `prepare()/write()` return `workflowFallback[{seq,reason}]`, echoed on the 4W create + UJP decision responses; 4W create accepts `routePlanId` (→ `routes.route_plan_id`); approve passes the pin and drops it on the PICKUP force-flip. CR-5b (§21.2): `GET /v1/route-plans/:id` drift `field: 'workflow'`. Tariff (§21.3): 409 `TARIFF_OVERLAP` on create/update (`extentsOverlap`), `RING_AUTO_SINGLE` note, `tariff_rings.active` (migration **`0099_tariff_ring_active`**), `PATCH …/rings/:id { active }`. Masters admin (§21.4): `POST/PATCH /v1/ujp/masters/vehicles[/:id]`, `POST /v1/ujp/masters/energy-prices` + `GET …/history`, `POST/PATCH /v1/ujp/masters/subcon-vendors[/:id]`; list endpoints `includeInactive` + `active`. ERD updated in the same change.
- 2026-09-20 — **v3.2**: change request CR-4b (assessment §20, CR4b-D1…D13) — full tariff parity. Migration **`0098_tariff_full`**: `tariff_configs` + `DISTANCE_TIER`, `rates_flat_per_body(_multi)` jsonb, `driver/helper_share_flat(_multi)`, `harga_reverse`, `hari_operasi`/`shift_operasi` jsonb, `tagih_per_rit`, `distance_tiers` jsonb, `multidrop_free_points`/`multidrop_extra_rate`, `catatan`; `tariff_rings` + `rates_per_body(_multi)`, `driver/helper_share(_multi)`, `keterangan`; `ujp.komisi_amount`. `RevenueService` v3 prices per §20.3 (body-aware ladder BODY_MULTI > BASE_MULTI > BODY_SINGLE > BASE_SINGLE, tier lookup inclusive on `maxKm`, surcharge `max(0, drops − free) × rate` via exact `MoneyHelper.multiplyInt`, reverse, komisi for a DRIVER payee only, `margin = revenue − cost − komisi`), returns `komisiAmount` and an extended frozen `tariff` snapshot (`rateApplied`, `rateSource`, `bodyKey`, `tier`, `surcharge`, `reverseAmount`, `komisi`, `notes[]`); `revenue_note` becomes a `,`-joined list; `revenue_status` gains `MISSING_TIER`. `TariffRepository.findActiveForClient(clientId, date, shift)` applies the pure `matchesOperatingWindow` (weekday read off the date string via UTC). `/v1/tariffs*` DTOs accept every new field (money-record validator, nested tier DTO; tiers normalised + duplicate `maxKm` rejected 400). `suggestRingId` = port of `ring-match.ts`. `GET /v1/ujp/:id` `revenue` gains `komisi`; the decision response gains `komisiAmount`. Web: `/ujp/tariffs` editors for tiers / per-body / shares / surcharge / hari-shift, breakdown in the approver panel, keyword ring suggest. Unit: oracle edges (§20.4), window predicate, matcher; integration: DISTANCE_TIER round-trip, window miss/hit, per-body ring + komisi. ERD updated in the same change.
- 2026-09-18 — **v3.1**: change request CR-4 (§14.5, CR4-D1…D10) — the revenue side. New **`tariff` module** (`tariff_configs` + config-scoped `tariff_rings`, `/v1/tariffs*`, `RevenueService`) imported by `ujp`; migration **`0097_tariff`** + `TARIFF_TIPE` enum adds the two tables, seven `ujp` columns (`tariff_config_id`, `ring_id`, `revenue_amount`, `margin_amount`, `revenue_status`, `revenue_note`, `tariff`) and `route_plans.default_ring_id`. The UJP tags tariff + ring at create (`POST`/`PUT` bodies) and **resolves + snapshots revenue and `margin = revenue − cost` at approve on one basis** (§2.10); revenue is **never fabricated** (`revenue_status` gates it). The response mapper splits into **field tiers** — cost tier (requester + approver) and **price tier (approver only)**, a critical masking test; `GET /v1/ujp/:id` gains `route.tariffConfigId`/`ringId`/`ringName`, the approver-only `revenue` block and `viewer.isPriceViewer`. The decision endpoint's approve response gains `revenue*` and a `409 NEGATIVE_MARGIN_CONFIRM_REQUIRED` guarded by `confirmNegativeMargin` (CR4-D7). Web gains a `/ujp/tariffs` page, a wizard ring picker with auto-suggest, the approver-only margin section + `revenueStatus` banners, and a missing-ring queue badge. Supersedes the out-of-scope "Ring / PER_RING tariff"; CR-4b (per-body/tier rates, distance tiers, driver/helper shares, margin dashboard) deferred. ERD updated in the same change. Supersedes nothing structurally — additive on top of CR-2/CR-3.
