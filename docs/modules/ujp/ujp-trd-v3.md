...
| `GET /v1/ujp/riders/:riderId/last-bank` | WEB | Rekening prefill for the Driver step: latest non-cancelled DRIVER-payee UJP for `rider.id` (APPROVED first, then newest) → `{ data: { bankName, accountNumber, accountHolder, sourceReferenceId, sourceDeliveryDate } \| null }`. driver-service has no bank fields, so the rider's own last UJP is the source. Prefill only; any UJP user may read it. |
...
- 2026-09-21 — **v3.4.1**: dead inputs dropped and a rekening prefill added. `ujp.ops_team`, `ujp.jam_mulai`, `ujp.jam_selesai` and the six cargo columns (`sender_name/phone`, `receiver_name/phone`, `item_name`, `bobot`) dropped — the `cargo` DTO group and `GET` block are gone (nothing read them: the 4W shipment takes sender/receiver from the stops). New `GET /v1/ujp/riders/:riderId/last-bank` (`UjpRepository.findLastForRider`, `GetUjpRiderBankUseCase`) feeds the wizard's bank block from the driver's last UJP; web `applyRiderBank` fills only an untouched block. Also (`header.jamMulai/jamSelesai` removed from DTOs and responses; they were logisticdash schedule fields with no reader here) (the requester is `requester_email` from auth; no ops-team master). `header.opsTeam` removed from `POST/PUT /v1/ujp` and from `GET /v1/ujp[/:id]` responses; the edit change-list no longer tracks it. Folded into the single feature migration **`0097_ujp_cr1_to_cr6`** (regenerated) rather than a follow-up migration, since the branch is unmerged.
