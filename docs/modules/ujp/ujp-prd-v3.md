...
2. The wizard has five steps with **forward-only dependencies**: Info (client, tanggal kirim, layanan, tipe pengiriman, shift) → Rute (route plan → stops, km) → Biaya (payee, vehicle, e-money, cost lines) → Driver (rider, rekening — prefilled from the driver's last UJP, editable) → Review.
