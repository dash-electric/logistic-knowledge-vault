...
- 2026-09-21 — v3.6: **CR-8 — leg pool: ditagih atau tidak diputuskan per stop, dan stop pool selalu ikut ke shipment.** On a pool stop the planner asks "Ditagih ke client?" with *Ikuti aturan client* as the default; ops may override either way for one trip, and the approver sees a "Beda dari aturan client" badge. Billing and the route are now separate questions: the driver always gets the pool stops (so the time to and at the pool is recorded), only the km may be excluded from the bill. Decisions CR8-D1…D6 in `ASSESSMENT-UJP-PORT-4W.md` §24.
